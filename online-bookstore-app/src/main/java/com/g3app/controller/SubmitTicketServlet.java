...
public class SubmitTicketServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            // Create new support ticket object
            SupportTicket ticket = new SupportTicket();
            ticket.setCustomerName(customer_name);
            ticket.setEmail(email);
            ticket.setSubjectTitle(subject_title);
            ticket.setTypeOfEnquiry(type_of_enquiry);
            ticket.setIssueDescription(issue_description);
            ticket.setStatus("Open");  // Default status is 'Open'
            ticket.setDateSubmitted(new Date());  // Set the current date as the submission date
